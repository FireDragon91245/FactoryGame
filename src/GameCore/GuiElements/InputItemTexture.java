...
public final class InputItemTexture implements DynamicGuiElement {
    ...
    public void drawDynamic(Graphics2D g2, Building b, int x, int y) {
        if(b.getInventory().getInputSlot(slot).getItemType() != Items.None && b.getInventory().getInputSlot(slot).getItemType() != Items.Blocked){
            g2.drawImage(Main.getClient().clientGraphics().gui().getItemTexture(b.getInventory().getInputSlot(slot).getItemType()), x + startX, y + startY, width, height, Main.getClient().clientGraphics().getMainObserver());
            ...
            if (cacheFontMetrics) {
                ...
                g2.drawString(String.valueOf(b.getInventory().getInputSlot(slot).getItemCount()), x + startX + width - g2.getFontMetrics(Main.getClient().clientGraphics().gui().getNumberedFont()).stringWidth("0".repeat(charCount)) + textOffsetX, y + startY + height + textOffsetY);
            }
        }
    }
}
